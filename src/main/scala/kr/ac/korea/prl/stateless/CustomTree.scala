...
case class CustomInit(
    tpe: CustomType,
    name: CustomName,
    argss: List[List[CustomTerm]]
) extends CustomRef
...
/* ============ Member ============ */
case class CustomSelf(name: CustomName, decltpe: Option[CustomType])
    extends Member {
  def apply(name: CustomName, decltpe: Option[CustomType]) =
    new CustomSelf(name, decltpe)
}
...
case class TypeBounds(lo: Option[CustomType], hi: Option[CustomType])
    extends CustomType {
  def apply(lo: Option[CustomType], hi: Option[CustomType]) =
    new TypeBounds(lo, hi)
}
...
case class TypeApply(tpe: CustomType, arg: List[CustomType])
    extends CustomType {
...
case class CustomCase(
    pat: CustomPat,
    cond: Option[CustomTerm],
    body: CustomTerm
) extends CustomTree {
  def apply(pat: CustomPat, cond: Option[CustomTerm], body: CustomTerm) =
    new CustomCase(pat, cond, body)
}

/* ============ CustomEnumerator ============ */
sealed trait CustomEnumerator extends CustomTree
case class Generator(pat: CustomPat, rhs: CustomTerm) extends CustomEnumerator {
  def apply(pat: CustomPat, rhs: CustomTerm) = new Generator(pat, rhs)
}

case class CaseGenerator(pat: CustomPat, rhs: CustomTerm)
    extends CustomEnumerator {
  def apply(pat: CustomPat, rhs: CustomTerm) = new CaseGenerator(pat, rhs)
}

case class Val(pat: CustomPat, rhs: CustomTerm) extends CustomEnumerator {
...
case class TermParam(
  ...
  def apply(
      ...
      decltpe: Option[CustomType],
      default: Option[CustomTerm]
  ) = new TermParam(mods, name, decltpe, default)
}

case class TermLambda(params: List[TermParam], body: CustomTerm)
    extends CustomTerm {
  def apply(params: List[TermParam], body: CustomTerm) =
    new TermLambda(params, body)
}
...
case class TermInterpolate(
    prefix: TermName,
    parts: List[CustomLit],
    args: List[CustomTerm]
) extends CustomTerm {
  def apply(prefix: TermName, parts: List[CustomLit], args: List[CustomTerm]) =
    new TermInterpolate(prefix, parts, args)
}

case class TermApply(fun: CustomTerm, args: List[CustomTerm])
    extends CustomTerm {
  def apply(fun: CustomTerm, args: List[CustomTerm]) = new TermApply(fun, args)
}

case class TermApplyUsing(fun: CustomTerm, args: List[CustomTerm])
    extends CustomTerm {
  def apply(fun: CustomTerm, args: List[CustomTerm]) =
    new TermApplyUsing(fun, args)
}

case class TermApplyType(fun: CustomTerm, targs: List[CustomType])
    extends CustomTerm {
  def apply(fun: CustomTerm, targs: List[CustomType]) =
    new TermApplyType(fun, targs)
}

case class TermApplyInfix(
    lhs: CustomTerm,
    op: TermName,
  ...
  def apply(
      ...
      targs: List[CustomType],
      args: List[CustomTerm]
  ) = new TermApplyInfix(lhs, op, targs, args)
}
...
case class TermIf(cond: CustomTerm, thenp: CustomTerm, elsep: CustomTerm)
    extends CustomTerm {
  def apply(cond: CustomTerm, thenp: CustomTerm, elsep: CustomTerm) =
    new TermIf(cond, thenp, elsep)
}

case class TermTry(
    expr: CustomTerm,
    catchp: List[CustomCase],
    finallyp: Option[CustomTerm]
) extends CustomTerm {
  def apply(
      expr: CustomTerm,
      catchp: List[CustomCase],
      finallyp: Option[CustomTerm]
  ) = new TermTry(expr, catchp, finallyp)
}
...
case class TermFor(enums: List[CustomEnumerator], body: CustomTerm)
    extends CustomTerm {
  def apply(enums: List[CustomEnumerator], body: CustomTerm) =
    new TermFor(enums, body)
}
...
case class DefVal(
  ...
  def apply(
      ...
      decltpe: Option[CustomType],
      rhs: CustomTerm
  ) = new DefVal(mods, pats, decltpe, rhs)
}
...
case class DefVar(
  ...
  def apply(
      ...
      decltpe: Option[CustomType],
      rhs: Option[CustomTerm]
  ) = new DefVar(mods, pats, decltpe, rhs)
}
...
case class DefDef(
  ...
  def apply(
      ...
      decltpe: Option[CustomType],
      body: CustomTerm
  ) = new DefDef(mods, name, tparams, paramss, decltpe, body)
}
...
case class DefEnum(
  ...
  def apply(
      ...
      ctor: PrimaryCtor,
      templ: CustomTemplate
  ) = new DefEnum(mods, name, tparams, ctor, templ)
}
...
case class DefObject(
    mods: List[CustomMod],
    name: TermName,
    templ: CustomTemplate
) extends CustomDefn {
  def apply(mods: List[CustomMod], name: TermName, templ: CustomTemplate) =
    new DefObject(mods, name, templ)
}
...
case class PrimaryCtor(
  ...
  def apply(
      ...
      name: CustomName,
      paramss: List[List[TermParam]]
  ) = new PrimaryCtor(mods, name, paramss)
}

case class SecondaryCtor(
    mods: List[CustomMod],
    name: CustomName,
    paramss: List[List[TermParam]],
    init: CustomInit,
  ...
  def apply(
      ...
      init: CustomInit,
      stats: List[CustomStat]
  ) = new SecondaryCtor(mods, name, paramss, init, stats)
}

/* ============ CustomTemplate ============ */
case class CustomTemplate(
    early: List[CustomStat],
  ...
  def apply(
      ...
      self: CustomSelf,
      stats: List[CustomStat]
  ) = new CustomTemplate(early, inits, self, stats)
}
...
case class CustomImporter(ref: TermRef, importees: List[CustomImportee])
    extends CustomTree {
  def apply(ref: TermRef, importees: List[CustomImportee]) =
    new CustomImporter(ref, importees)
}
